...
def precommit(session: Session) -> None:
    ...
    session.run("rm", "-rf", "~/.cache/flakehell", external=True)
...
def tests(session: Session) -> None:
    """Run the test suite."""
    session.install(".")
    session.install("coverage[toml]", "pytest", "pygments", "xdoctest[colors]")
    try:
        session.run("coverage", "run", "--parallel", "-m", "pytest", "--xdoctest", *session.posargs)
    finally:
...
# @session(python=python_versions)
# def snyk(session: Session) -> None:
#     """Test security dependencies with snyk."""
#     session.install(".")
#     session.install("pytest", "typeguard", "pygments")
#     session.run("pytest", f"--typeguard-packages={package}", *session.posargs)
